...
def generate_tour_bracket(tour: Tournament):
    ...
    players = list(
        TournamentParticipant.objects.filter(tournament=tour).values_list("player", flat=True)
    )
    ...
    for i in range(0, size, 2):
        ...
        m = Match.objects.create(tournament=tour, round_number=round_no, player1_id=p1, player2_id=p2)
    ...
    while len(current_round) > 1:
        ...
        for i in range(0, len(current_round), 2):
            ...
            m_next = Match.objects.create(tournament=tour, round_number=round_no, prev_match_1=m1, prev_match_2=m2)
